...
ssl_options(Endpoint) ->
    VerifyOpts =
        case application:get_env(vmq_webhooks, verify_peer) of
            {ok, false} ->
                [];
            _ ->
                VerifyFun = {
    ...
    CACertFileOpts =
        case application:get_env(vmq_webhooks, cafile) of
            {ok, ""} -> [];
            {ok, CACF} -> [{cacertfile, CACF}];
            undefined -> []
        end,
    CRLOpts =
        case application:get_env(vmq_webhooks, use_crls) of
            {ok, false} ->
                [];
            _ ->
                [
    ...
    CertFileOpts =
        case application:get_env(vmq_webhooks, certfile) of
            {ok, ""} -> [];
            {ok, CF} -> [{certfile, CF}];
            undefined -> []
        end,
    KeyFileOpts =
        case application:get_env(vmq_webhooks, keyfile) of
            {ok, ""} -> [];
            {ok, KF} -> [{keyfile, KF}];
            undefined -> []
        end,
    KeyfilePasswordOpts =
        case application:get_env(vmq_webhooks, keyfile_password) of
            {ok, ""} -> [];
            {ok, KFP} -> [{password, KFP}];
            undefined -> []
        end,
